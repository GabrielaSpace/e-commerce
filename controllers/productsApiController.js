...
const getProducts = async (req, res) => {
    let products = [];
    const pagination =
        req.query.hasOwnProperty("page") && req.query.hasOwnProperty("limit");
    try {
        if (pagination) {
            //Pagination
...
const getProduct = async (req, res) => {
    ...
    try {
        let product = await Product.findOne({ title: title }, "-_id -__v");
...
const createProduct = async (req, res) => {
    ...
    let company_id = await Provider.findOne(
        { company_name: req.body.provider },
        "_id"
    );
